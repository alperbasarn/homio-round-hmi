...
static const uint8_t char_prop_notify __attribute__((unused)) = ESP_GATT_CHAR_PROP_BIT_NOTIFY;
...
static const uint8_t char_prop_read_write_notify __attribute__((unused)) = ESP_GATT_CHAR_PROP_BIT_READ|ESP_GATT_CHAR_PROP_BIT_WRITE|ESP_GATT_CHAR_PROP_BIT_NOTIFY;
...
static uint16_t bas_att_tbl[BAS_IDX_NB] = {0};
    ...
    switch(event) {
        ...
        case ESP_GATTS_CREAT_ATTR_TAB_EVT: {
            if (param->add_attr_tab.num_handle == BAS_IDX_NB &&
                ...
                memcpy(bas_att_tbl, param->add_attr_tab.handles, sizeof(bas_att_tbl));
...
static void gatts_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if,
                                esp_ble_gatts_cb_param_t *param)
{
    extern void bluetooth_manager_extra_gatts_event_handler(esp_gatts_cb_event_t event,
                                                            esp_gatt_if_t gatts_if,
                                                            esp_ble_gatts_cb_param_t *param)
        __attribute__((weak));

    if (bluetooth_manager_extra_gatts_event_handler != NULL) {
        bluetooth_manager_extra_gatts_event_handler(event, gatts_if, param);
    }
...
void hidd_set_battery_level(uint8_t percentage)
{
    battary_lev = percentage;

    if (bas_att_tbl[BAS_IDX_BATT_LVL_VAL] == 0) {
        return;
    }

    esp_ble_gatts_set_attr_value(bas_att_tbl[BAS_IDX_BATT_LVL_VAL], sizeof(battary_lev), &battary_lev);

    if (!hidd_le_env.hidd_clcb[0].connected || bas_att_tbl[BAS_IDX_BATT_LVL_NTF_CFG] == 0) {
        return;
    }

    uint16_t cccLen = 0;
    uint8_t* cccValue = NULL;
    esp_ble_gatts_get_attr_value(bas_att_tbl[BAS_IDX_BATT_LVL_NTF_CFG], &cccLen, (const uint8_t**)&cccValue);
    if (cccLen < 2 || cccValue == NULL || (cccValue[0] & 0x01) == 0) {
        return;
    }

    esp_ble_gatts_send_indicate(
        hidd_le_env.gatt_if,
        hidd_le_env.hidd_clcb[0].conn_id,
        bas_att_tbl[BAS_IDX_BATT_LVL_VAL],
        sizeof(battary_lev),
        &battary_lev,
        false);
}
