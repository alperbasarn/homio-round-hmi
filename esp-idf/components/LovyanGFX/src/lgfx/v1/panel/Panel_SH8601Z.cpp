...
namespace lgfx
{
    inline namespace v1
    {
        ...
        bool Panel_SH8601Z::init(bool use_reset)
        {
            ...
            // Match the vendor init sequence used by the official ESP32-C6 AMOLED example.
            uint8_t cmds[] =
            {
                0x11, 0 + CMD_INIT_DELAY, 80,   // Sleep out
                0xC4, 1, 0x80,
                0x53, 1 + CMD_INIT_DELAY, 0x20, 1,
                0x63, 1 + CMD_INIT_DELAY, 0xFF, 1,
                0x51, 1 + CMD_INIT_DELAY, 0x00, 1, // Brightness low before display on
        ...
        void Panel_SH8601Z::setWindow(uint_fast16_t xs, uint_fast16_t ys, uint_fast16_t xe, uint_fast16_t ye)
        {
            ...
            // Waveshare 1.43" SH8601 panel maps visible X to controller X+6.
            constexpr uint16_t x_offset = 6;

            /* Set limit */
            if ((xe - xs) >= _width) { xs = 0; xe = _width - 1; }
            if ((ye - ys) >= _height) { ys = 0; ye = _height - 1; }

            // SH8601 on this board is most stable when updates use even start and odd end.
            xs &= ~1;
            ys &= ~1;
            xe |= 1;
            ye |= 1;
            if (xe >= _width)  xe = _width  - 1;
            if (ye >= _height) ye = _height - 1;
